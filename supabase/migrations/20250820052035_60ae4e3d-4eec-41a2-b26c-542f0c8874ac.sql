...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM storage.policies WHERE name = 'public_read_images') THEN
    CREATE POLICY "public_read_images" ON storage.objects 
    FOR SELECT USING (bucket_id = 'images');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM storage.policies WHERE name = 'authenticated_write_images') THEN
    CREATE POLICY "authenticated_write_images" ON storage.objects 
    FOR INSERT WITH CHECK (bucket_id = 'images' AND auth.role() = 'authenticated');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM storage.policies WHERE name = 'authenticated_update_images') THEN
    CREATE POLICY "authenticated_update_images" ON storage.objects 
    FOR UPDATE USING (bucket_id = 'images' AND auth.role() = 'authenticated');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM storage.policies WHERE name = 'authenticated_delete_images') THEN
    CREATE POLICY "authenticated_delete_images" ON storage.objects 
    FOR DELETE USING (bucket_id = 'images' AND auth.role() = 'authenticated');
  END IF;
END $$;

-- Storage policies for docs bucket (if not exists)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM storage.policies WHERE name = 'public_read_docs') THEN
    CREATE POLICY "public_read_docs" ON storage.objects 
    FOR SELECT USING (bucket_id = 'docs');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM storage.policies WHERE name = 'authenticated_write_docs') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM storage.policies WHERE name = 'authenticated_update_docs') THEN
    CREATE POLICY "authenticated_update_docs" ON storage.objects 
    FOR UPDATE USING (bucket_id = 'docs' AND auth.role() = 'authenticated');
  END IF;

  IF NOT EXISTS (SELECT 1 FROM storage.policies WHERE name = 'authenticated_delete_docs') THEN
