...
export default function AdminNews() {
  ...
  const { data: itemsData, isLoading } = useQuery({
    queryKey: ['admin', 'news', selectedLocale],
    queryFn: () => adminApi.listNews({ locale: selectedLocale, pageSize: 50 }),
  });

  const createMutation = useMutation({
    mutationFn: (data: any) => adminApi.createNews(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => adminApi.updateNews(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin', 'news'] });
      ...
      <div className="flex justify-between items-center">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingItem ? 'Edit News' : 'Create News Article'}</DialogTitle>
              <DialogDescription>
                {editingItem ? 'Update the news article' : 'Add a new news article'}
              </DialogDescription>
            </DialogHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select name="status" defaultValue={editingItem?.status || 'draft'} required>
                    <SelectTrigger>
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>Manage your news content</CardDescription>
            </div>
